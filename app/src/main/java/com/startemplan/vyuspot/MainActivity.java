...
public class MainActivity extends Activity {
	...
	public static final String NUM = "contactNumber";
	public static final String REG_ID = "regId";
	public static final String EMAIL_ID = "eMailId";
	EditText emailET,nameET,phoneET;
...
*//*
	...
	public void RegisterUser(View view) {
		...
		String nameID = nameET.getText().toString();
		String phoneID = phoneET.getText().toString();

		if (!TextUtils.isEmpty(emailID) && Utility.validate(emailID) && !TextUtils.isEmpty(nameID) && !TextUtils.isEmpty(phoneID)) {
			// Check if Google Play Service is installed in Device
			// Play services is needed to handle GCM stuffs
			if (checkPlayServices()) {

				// Register Device in GCM Server
				registerInBackground(emailID,nameID,phoneID);
			}
		}
	...
	private void registerInBackground(final String emailID,final String nameID,final String phoneID) {
		new AsyncTask<Void, Void, String>() {
			...
			protected void onPostExecute(String msg) {
				if (!TextUtils.isEmpty(regId)) {
					storeRegIdinSharedPref(applicationContext, regId,nameID,phoneID, emailID);
					Toast.makeText(
							applicationContext,
	...
	private void storeRegIdinSharedPref(Context context, String regId,String nameId, String phoneId,String emailID) {
		SharedPreferences prefs = getSharedPreferences("UserDetails",
				Context.MODE_PRIVATE);
		SharedPreferences.Editor editor = prefs.edit();
		editor.putString(REG_ID, regId);
		editor.putString(NAME, nameId);
		editor.putString(NUM,phoneId);
		editor.putString(EMAIL_ID, emailID);
		editor.commit();
		storeRegIdinServer(regId, emailID,nameId,phoneId);

	}

	// Share RegID and Email ID with GCM Server Application (Php)
	private void storeRegIdinServer(String regId2, String emailID,String nameId,String phoneId) {
		prgDialog.show();
		params.put("emailId", emailID);
		params.put("regId", regId);
		params.put("Name", nameId);
		params.put("contactNumber", phoneId
		);
		Log.d("tag3",emailID+regId2);
